...
bool loadComputeShader(const std::string& name, uint32_t& compShader)
{
	std::string shaderData;
	if (!readShader(name, shaderData)) {
		return false;
	...
	const char* src = shaderData.c_str();
	glShaderSource(compShader, 1, &src, nullptr);
	glCompileShader(compShader);
	int res;
	glGetShaderiv(compShader, GL_COMPILE_STATUS, &res);
	if (res == GL_FALSE) {
		readShaderError(compShader, GL_FRAGMENT_SHADER);
		return false;
	}
...
uint32_t attachShaders(uint32_t vertexShader, uint32_t geometry, uint32_t pixelShader)
{
	uint32_t p = glCreateProgram();
	glAttachShader(p, vertexShader);
