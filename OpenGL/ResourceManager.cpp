...
uint32_t ResourceManager::getShader(const std::string& ShaderFile)
{
    if(Shaders.find(ShaderFile) == Shaders.end()){
        ...
        if(extension == "vert"){
            ...
            if (!loadComputeShader(ShaderFile, newShader)) {
                std::cout << "couldn't create shader" << std::endl;
            }
        }
        else{
...
uint32_t ResourceManager::createShaderProgram(const std::string& ShaderProgramName, uint32_t vertex, uint32_t geometry, uint32_t pixel)
{
    uint32_t shaderProgram = attachShaders(vertex, geometry, pixel);
    ShaderProgram.insert(std::make_pair(ShaderProgramName, shaderProgram));
    return shaderProgram;
}
