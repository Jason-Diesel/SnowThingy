...
template <typename T>
uint32_t CreateParticleBuffer(std::vector<T>& vertecies, GLenum type = GL_FLOAT) {
    uint32_t buffer = 0;
    GLTest(glGenBuffers(1, &buffer));
    GLTest(glBindBuffer(GL_ARRAY_BUFFER, buffer));
    GLTest(glBufferData(GL_ARRAY_BUFFER, sizeof(T) * vertecies.size(), vertecies.data(), GL_DYNAMIC_DRAW));

    GLTest(glEnableVertexAttribArray(0));
    GLTest(glVertexAttribPointer(0, 3, type, GL_FALSE, sizeof(T), (void*)offsetof(T, m_position)));
    GLTest(glEnableVertexAttribArray(1));
    GLTest(glVertexAttribPointer(1, 2, type, GL_FALSE, sizeof(T), (void*)offsetof(T, m_velocity)));
    GLTest(glEnableVertexAttribArray(2));
    GLTest(glVertexAttribPointer(2, 3, type, GL_FALSE, sizeof(T), (void*)offsetof(T, m_normals)));
